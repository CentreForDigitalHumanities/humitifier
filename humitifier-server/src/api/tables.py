...
class OAuth2ApplicationsTable(BaseTable):
    class Meta:
        ...
        no_data_message_wild_wasteland = ("No apps found... What is an "
                                          "Application anyway?")

    allowed_scopes = MethodColumn(
        "Allowed scopes",
        method_name='get_allowed_scopes'
    )
    ...
    actions = CompoundColumn(
        ...
        columns=[
            ButtonColumn(
                ...
                url=lambda obj: reverse('api:edit_oauth_application',
                                        args=[obj.pk]),
            ),
            ButtonColumn(
                text='Delete',
                button_class='btn btn-danger',
                url=lambda obj: reverse('api:delete_oauth_application',
                                        args=[obj.pk]),
            ),
        ],
    )