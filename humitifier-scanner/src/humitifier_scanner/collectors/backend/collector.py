...
class WindowsShellCollector(Collector):
    """
    Provides a base class for windows-shell-based collectors, as a shorthand class
    for collectors using powershell-executors only.

    The purpose of this class is to facilitate creating collectors that
    rely on windows shell executors for their functionality. It provides a premade
    `collect` method that automatically retrieves the shell executor
    from the `info` argument and invokes the `collect_from_shell` method,
    which must be implemented by subclasses to define the specific logic
    for collecting data using shell commands.
    ...
    required_executors = [Executors.WINDOWS_SHELL]
    ...
    def collect(self, info: CollectInfo) -> T:
        ...
        `info` argument.
        """
        executor: WindowsShellExecutor = info.executors.get(Executors.WINDOWS_SHELL)
        if not executor:
            raise ValueError("Windows Shell executor is required for this collector")
    ...
    def collect_from_shell(
        ...
        """Implement your windows-shell-based collector logic here."""
        raise NotImplementedError(
            "Collector must implement a collect_from_shell method"
        )
